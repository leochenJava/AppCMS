...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="appController/appversionadd?appInfoId="+obj.attr("appinfoid");
	//open("appController/appversionadd?appInfoId="+obj.attr("appinfoid"))
...
$(".modifyAppInfo").on("click",function(){
	...
	var appinfoid = obj.attr("appinfoid");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="appController/modify/"+ appinfoid
	}else{
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	var appinfoid = obj.attr("appinfoid");
	window.location.href="appController/appview/"+ appinfoid;
});
